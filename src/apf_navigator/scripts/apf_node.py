...
class APFNavigator:
    def __init__(self):
        ...
        self.D_s = rospy.get_param("~D_s", 2.0)
        ...
        self.obstacles = []
        self.last_valid_w = 0.0  # 上一次有效角速度
        ...
        # 发布速度控制指令
        self.cmd_pub = rospy.Publisher("/am_nav/cmd_vel", Twist, queue_size=1)
    ...
    def control_loop(self, event):
        ...
        w = self.k_w * angle
        if np.isnan(w):
            rospy.logwarn("⚠️ 当前角速度无效，使用上一次有效角速度: w=%.2f", self.last_valid_w)
            w = self.last_valid_w
        else:
            self.last_valid_w = w
